import { ChatAdapter, IChatGroupAdapter, Group, Message, ChatParticipantStatus, ParticipantResponse, ChatParticipantType, IChatParticipant, MessageType } from 'projects/hss-chat/src/public-api';
...
export class DemoAdapter extends ChatAdapter implements IChatGroupAdapter
{
    ...
    public listFriendsByPage(size: number, page: number) : Observable<ParticipantResponse[]> {
        return of(DemoAdapter.mockedParticipants.map(user => {
            let participantResponse = new ParticipantResponse();
            participantResponse.participant = user;
            participantResponse.metadata = {
